    ...
    I: I2c<Error = E>,
{
    ...
    /// # use embedded_hal_mock::eh1::{delay::NoopDelay, i2c::Mock};
    /// # use bmp390::Bmp390;
    /// # async fn run() -> Result<(), bmp390::Error<embedded_hal::i2c::ErrorKind>> {
    /// # let config = bmp390::Configuration::default();
    /// # let i2c = embedded_hal_mock::eh1::i2c::Mock::new(&[]);
    /// # let delay = embedded_hal_mock::eh1::delay::NoopDelay::new();
    /// # let mut sensor = Bmp390::try_new(i2c, bmp390::Address::Up, delay, &config).await?;
    /// let fifo_length = sensor.fifo_length().await?;
    /// defmt::info!("Length: {}", fifo_length);
    /// # Ok(())
    /// # }
    /// ```
    pub async fn fifo_length(&mut self) -> Result<u16, Error<E>> {
    ...
    /// # let mut sensor = Bmp390::try_new(i2c, bmp390::Address::Up, delay, &config).await?;
    /// let mut data = [0; 512];
    /// sensor.read_fifo(&mut data).await?;
    /// defmt::info!("Data: {=[u8]:#04x}", data);
    /// # Ok(())
    /// # }
    /// ```
    pub async fn read_fifo<const N: usize>(&mut self, data: &mut [u8; N]) -> Result<(), Error<E>> {
    ...
    /// # let delay = embedded_hal_mock::eh1::delay::NoopDelay::new();
    /// # let mut sensor = Bmp390::try_new(i2c, bmp390::Address::Up, delay, &config).await?;
    /// let iter = sensor.iter_fifo().await?;
    /// for frame in iter {
    ...
    /// # }
    /// ```
    pub async fn iter_fifo(&mut self) -> Result<fifo::FifoIter<core::array::IntoIter<u8, 512>>, Error<E>> {
        let mut frames = [0; 512];
        self.read_fifo(&mut frames).await?;
    ...
    /// # let delay = embedded_hal_mock::eh1::delay::NoopDelay::new();
    /// # let mut sensor = Bmp390::try_new(i2c, bmp390::Address::Up, delay, &config).await?;
    /// sensor.flush_fifo().await?; // FIFO is now empty
    /// # Ok(())
    /// # }
    /// ```
    pub async fn flush_fifo(&mut self) -> Result<(), Error<E>> {
        self.command(Command::FifoFlush).await
    }

    /// Sends a command to the sensor.
    async fn command(&mut self, command: Command) -> Result<(), Error<E>> {
